...
export const createProductController = async (req, res) => {
  try {
    ...
    switch (true) {
      ...
      case !category:
        return res.status(500).send({ error: "Catogery is required" });
      case !quantity:
        return res.status(500).send({ error: "Quantity is required" });
      case photo && photo.size > 1000000:
        return res
          .status(500)
          .send({ error: "Photo is required and shoul be less then 1mb" });
    }
...
export const productCategoryController = async (req, res) => {
  try {
    ...
    res.status(400).send({
      success: false,
      message: "Error while handling category",
      error,
    });
  }
};
